...
void RobotROS::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& msg) {
    ...
    // Salva per pubblicazione odometria
    current_twist_ = *msg;
}

void RobotROS::timeTick(float dt) {
    // Chiama il metodo base per muovere il robot
    Robot::timeTick(dt);
    
    // Pubblica odometria e transform
    publishOdometry();
    publishTransform();
}
...
void RobotROS::publishOdometry() {
    ...
    // Calcola la pose nel mondo
    Pose world_pose = poseInWorld();
    
    // Crea messaggio odometria
    nav_msgs::Odometry odom_msg;
    odom_msg.header.stamp = current_time;
    odom_msg.header.frame_id = "map";  // Frame fisso del mondo
    ...
    // Velocità (usando i comandi correnti)
    odom_msg.twist.twist.linear.x = tv;
    odom_msg.twist.twist.linear.y = 0.0;
    odom_msg.twist.twist.angular.z = rv;
    
    // Covariance (valori di esempio - potrebbero essere calibrati)
    for (int i = 0; i < 36; i++) {
...
void RobotROS::publishTransform() {
    // Pubblica transform da map a robot frame
    Pose world_pose = poseInWorld();
